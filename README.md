...
1. **成员 B**：运行 `成员B文件/code/preprocessing.py` 生成 `成员B文件/data/cell_vectors.npy`
2. **成员 A**：运行 `成员A文件/main_a.py build` 构建并保存索引
3. **成员 C**：进入 `成员C文件` 运行后端 Flask 服务 `python app.py` 提供 API
4. **成员 D**：开启根目录下资源访问服 `python -m http.server 8000` 并使用浏览器打开前端界面 `http://localhost:8000/成员D文件/index.html`。
