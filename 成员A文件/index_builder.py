...
class IndexInfo:
    ...
    def __init__(self, method: str, n_cells: int, n_dims: int,
        ...
        self.method = method          # "faiss_ivf" / "hnsw"
        self.n_cells = n_cells        # 向量数
        self.n_dims = n_dims          # 向量维度
        self.build_time_s = build_time_s  # 构建耗时
        self.params = params          # 构建参数
...
class IndexManager:
    ...
    快速上手:
        ...
        mgr.build("faiss_ivf")          # 构建索引
        mgr.save("index_store/")        # 保存到磁盘
        mgr.load("index_store/", "faiss_ivf")  # 下次直接加载
        D, I = mgr.search(query, k=10)  # 检索
    """

    def __init__(self, vectors_path: str = "cell_vectors.npy"):
        """
        Args:
            vectors_path: 成员 B 导出的 cell_vectors.npy 路径
        """
        self.vectors_path = Path(vectors_path)
        self.vectors: Optional[np.ndarray] = None       # 原始向量
        self.vectors_norm: Optional[np.ndarray] = None  # L2 归一化向量
        self.n_cells: int = 0
        self.n_dims: int = 0
        ...
        self._active_method: Optional[str] = None      # 当前激活的索引方法
    ...
    def build(self,
        ...
        Args:
            ...
            nlist: [FAISS] 聚类中心数，建议 sqrt(n_cells)，默认 100
            nprobe: [FAISS] 搜索时探测的聚类数，越大越准但越慢
            hnsw_m: [HNSW] 每层最大连接数，建议 16~32
            hnsw_ef_construction: [HNSW] 构建时搜索宽度，越大越准但越慢
    ...
    def _build_faiss_ivf(self, nlist: int, nprobe: int) -> IndexInfo:
        """构建 FAISS IVF 索引（使用内积 = 余弦相似度）"""
        try:
            import faiss
        except ImportError:
            raise ImportError(
                "FAISS 未安装！请运行: pip install faiss-cpu\n"
                "（GPU 版本: pip install faiss-gpu）"
            )
    ...
    def load(self, save_dir: str = "index_store",
        ...
        Args:
            save_dir: 索引存储目录（与 save() 保持一致）
            method: 强制指定加载方法；None 则从 index_meta.json 自动读取
        """
        save_path = Path(save_dir)
        meta_path = save_path / "index_meta.json"
        ...
        if meta_path.exists():
            ...
            if method is None:
                raise FileNotFoundError(
                    f"找不到 index_meta.json，请手动指定 method 参数"
                )
    ...
    def search(self,
        ...
        Args:
            ...
            k: 返回 top-k 个最相似细胞
            ef_search: [HNSW] 查询精度，越大越准但越慢（默认 50）

        Returns:
            distances: shape (n_queries, k)，余弦相似度（越大越相似）
            indices:   shape (n_queries, k)，细胞索引（0-based）
        """
        if self._active_method is None:
    ...
    def _search_hnsw(self, q_norm: np.ndarray, k: int,
        ...
        # hnswlib 返回 L2 距离（cosine space），转为相似度
    ...
    def set_nprobe(self, nprobe: int) -> None:
        ...
        if self._faiss_index is not None:
            ...
            print("[IndexManager] 警告: FAISS 索引未构建，无法设置 nprobe")
    ...
    def set_ef_search(self, ef: int) -> None:
        ...
        if self._hnsw_index is not None:
            ...
            print("[IndexManager] 警告: HNSW 索引未构建，无法设置 ef_search")
...
def compare_index_methods(vectors_path: str = "cell_vectors.npy",
    ...
    """
    print("\n" + "=" * 70)
    print("  索引方法对比: FAISS IVF vs HNSW")
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    if not Path(VECTORS_PATH).exists():
        ...
        if Path(alt_path).exists():
            ...
            print("请将 cell_vectors.npy 放到当前目录，或修改 VECTORS_PATH 变量")
            sys.exit(1)
